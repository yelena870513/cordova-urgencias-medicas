<template>
  <v-ons-page>
    ...
    <v-ons-row v-show="!searchMode">
      <v-ons-col>
        <v-ons-card v-for="t of temas" :key="t.id" @click="push(t)">
          <div class="title">
            <div class="logo">
              <img :src="'./assets/images/logo/'+t.img" />
            </div>
            {{ t.titulo }}
          </div>
    ...
    <v-ons-row v-show="searchMode">
      <v-ons-col>
        <v-ons-card :key="index" v-for="(r, index) of searchResults" @click="setContent(r)">
          <div class="title">
            <h5>{{ r.titulo }}</h5>
          </div>
        ...
        <v-ons-card v-show="searchResults.length ===0">
          <div class="title">
            <h5>No hubo resultados para su búsqueda</h5>
          </div>
        </v-ons-card>
      </v-ons-col>
    </v-ons-row>
...
export default {
  ...
  methods: {
    push(item) {
      const contents = Object.values(this["multimedia/mContenido"]).filter(
        c => {
          return c.tema.id === item.id;
        }
      );
      if (contents.length > 1) {
          this.navigateToTopicList(item)
          
      } else {
          const reader = contents[0];
          this.navigateToContent(reader);
      }
    },
    setContent(item) {
    ...
    resolveHtml(html) {
      ...
      const res = last.replace(
        new RegExp(exp, "gi"),
        '<span class="highlightedText badge red">$&</span>'
      );
    ...
    ...mapActions([
      "multimedia/setTopic",
      "multimedia/setSearchMode",
      "multimedia/setReader",
      "multimedia/setSearchTerm"
    ])
  },
  computed: {
    temas() {
    ...
    searchResults() {
      ...
      if (search.length > 3) {
        let mResults = Object.assign(
          {},
          Resolver.filterItems(this["multimedia/contenido"], search)
        );
        return mResults;
      } else {
...
.text-input--material:focus {
  background-image: linear-gradient(rgb(0, 58, 82), rgb(0, 58, 82)),
